...
class DialogBertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, turn_ids=None, position_ids=None, role_ids=None, inputs_embeds=None):
        if input_ids is not None:
            input_shape = input_ids.size()
        else:
            input_shape = inputs_embeds.size()[:-1]
...
class DialogBertModel(BertPreTrainedModel):
    """
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """, BERT_START_DOCSTRING)
class DialogBertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class DialogBertForNextSentencePrediction(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
