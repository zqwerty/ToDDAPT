...
from .dialogbert_sampler import DialogBertSampler, DistributedDialogBertSampler
...
class Trainer:
    ...
    def __init__(
        ...
        self.bucket_sampling = False
        if buckets_id is not None:
            logger.info('using bucket sampling')
            self.bucket_sampling = True

        self.buckets_id = buckets_id

    def get_train_dataloader(self) -> DataLoader:
        if self.bucket_sampling:
            train_batch_sampler = DialogBertSampler(self.train_dataset, self.buckets_id, self.args.train_batch_size,
                                                    drop_last=False) if self.args.local_rank == -1 else DistributedDialogBertSampler
