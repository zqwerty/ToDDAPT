...
class PreTrainedTokenizer(SpecialTokensMixin):
    ...
    def num_special_tokens_to_add(self, sentences):
        """
        ...
        return len(self.build_inputs_with_special_tokens(sentences)) - sum([len(s) for s in sentences])
    ...
    def add_special_tokens(self, special_tokens_dict):
        ...
        Returns:
            Number of tokezns added to the vocabulary.
    ...
    def batch_encode_plus(
        ...
        **kwargs
        ...
        Args:
            line_examples (:obj:`List[str]`): contains lines of input text file
            add_special_tokens (:obj:`bool`, `optional`, defaults to :obj:`True`):
                If set to ``True``, the sequences will be encoded with the special tokens relative
                to their model.
            max_length (:obj:`int`, `optional`, defaults to :obj:`None`):
            ...
            stride (:obj:`int`, `optional`, defaults to ``0``):
                If set to a number along with max_length, the overflowing tokens returned will contain some tokens
                from the main sequence returned. The value of this argument defines the number of additional tokens.
            truncation_strategy (:obj:`str`, `optional`, defaults to `backwards`):
                String selected in the following options:
                - 'backwards': cut from further histories
                - 'do_not_truncate': Does not truncate (raise an error if the input sequence is longer than max_length)
            pad_to_max_length (:obj:`bool`, `optional`, defaults to :obj:`False`):
                If set to True, the returned sequences will be padded according to the model's padding side and
                padding index, up to their max length. If no max length is specified, the padding is done up to the
        ...
        input_ids = []
        for input_sentences in tqdm(line_examples, desc='splitting examples'):
            input_sentences = input_sentences.split('\t')
            assert len(input_sentences) % 2 == 0, print(len(input_ids), input_sentences)  # every line should contains N * (user, system)
            input_ids.append([get_input_ids(ids) for ids in input_sentences])

        if max_length is None and pad_to_max_length:

            def total_sequence_length(input_sentences):
                return self.num_special_tokens_to_add(input_sentences) + sum([len(s) for s in input_sentences])

            max_length = max([total_sequence_length(ids) for ids in input_ids])

        batch_outputs = {}
        for sentences in tqdm(input_ids, desc='processing data'):
    ...
    def get_num_utts(self, sequence):
        pass
    ...
    def prepare_for_model(
        ...
        Args:
            sentences: List[List[int]], all examples, every utterance is represented by a List[int]
            max_length: maximum length of the returned list. Will truncate by taking into account the special tokens.
            add_special_tokens: if set to ``True``, the sequences will be encoded with the special tokens relative
                to their model. if set to  ``False``, will raise
            truncation_strategy: string selected in the following options:
                - 'backwards' (default) truncate farthest history)
                - 'do_not_truncate': Does not truncate (raise an error if the input sequence is longer than max_length)
        ...
        if add_special_tokens:
            sequence = self.build_inputs_with_special_tokens(sentences)
            total_len = len(sequence)

            if max_length and len(sequence) > max_length:
                # logging.info('sequence truncated, length {}'.format(str(len(sequence))))
                # logging.info(' '.join(self.convert_ids_to_tokens(sequence)))
                ...
                assert len(sequence) <= max_length, print('sequence truncated, length {}'.format(str(len(sequence))))
                # logging.info(' '.join(self.convert_ids_to_tokens(sequence)))
        ...
        encoded_inputs['num_utts'] = self.get_num_utts(sequence)
        ...
        if return_special_tokens_mask:
            if add_special_tokens:
                encoded_inputs["special_tokens_mask"] = self.get_special_tokens_mask(sequence)
            else:
                encoded_inputs["special_tokens_mask"] = [0] * len(sequence)

        # Check lengths
        assert max_length is None or len(encoded_inputs["input_ids"]) <= max_length, print(max_length, len(encoded_inputs["input_ids"]))
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                ...
                if return_token_type_ids:
                    encoded_inputs["turn_ids"] = (
                        encoded_inputs["turn_ids"] + [self.pad_token_type_id] * difference
                    )
                    encoded_inputs['role_ids'] = (
                        encoded_inputs["role_ids"] + [self.pad_token_type_id] * difference
                    )
                    encoded_inputs['position_ids'] = (
                            encoded_inputs["position_ids"] + [self.pad_token_type_id] * difference
                    )
                if return_special_tokens_mask:
    ...
    def truncate_sequences(
        ...
        if truncation_strategy == 'backwards':
            sequence = sequence[:-num_tokens_to_remove-1] + sequence[-1:]  # take care of [sep] at the end
            overflowing_tokens = sequence[-num_tokens_to_remove-1: -1]
        elif truncation_strategy == "do_not_truncate":
            raise ValueError("Input sequence are too long for max_length. Please select a truncation strategy.")
        else:
            raise ValueError(
                "Truncation_strategy should be selected in ['backwards', 'do_not_truncate']"
            )
        return sequence, overflowing_tokens

    def create_token_type_ids_from_sequences(self, sequence) -> List[List[int]]:
        pass

    def build_inputs_with_special_tokens(self, sentences) -> List:
        """
    ...
    def get_special_tokens_mask(
        ...
        """
        all_special_ids = [self.convert_tokens_to_ids(tok) for tok in self.unique_added_tokens_encoder]
        ret = [0 if s not in all_special_ids else 1 for s in sequence]
        return ret
    ...
    def create_positional_ids_from_sequences(self, val):
        pass
