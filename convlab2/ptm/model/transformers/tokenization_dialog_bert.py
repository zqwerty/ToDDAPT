...
class DialogBertTokenizer(PreTrainedTokenizer):
    ...
    def get_num_utts(self, sequence):
        role_tokens = [self.added_tokens_encoder['[USR]'], self.added_tokens_encoder['[SYS]']]
        return sequence.count(role_tokens[0]) + sequence.count(role_tokens[1])


    def build_inputs_with_special_tokens(
        self, sentences: List[List[int]], prefix_dict: OrderedDict = None, utt_role='user'
    ) -> List[int]:
        """
    ...
    def get_special_tokens_mask_bak(
        ...
        for i, token_id in enumerate(sequence):
            if token_id in all_special_token_dict:
                if all_special_token_dict[token_id] == '[USR]' and is_first_turn == False:
                    is_first_turn = True
                elif all_special_token_dict[token_id] == '[USR]' and is_first_turn == True:
                    break
    ...
    def create_token_type_ids_from_sequences(self, sequence) -> [List[int], List[int]]:
        """
