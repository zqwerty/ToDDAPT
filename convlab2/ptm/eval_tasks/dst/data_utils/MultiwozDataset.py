...
class MultiwozDataset(Dataset):
    ...
    def preprocess_one_turn(self, turn, history_ids, history_utts):
        ...
        cur_utt_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(turn["utterance"]))
    ...
    def preprocess(self, data):
        ...
        for dial in tqdm(data[:int(len(data) * self.ratio)], desc="Preprocessing", ncols=80):
            ...
            for turn in dial["turns"]:
                ...
                for domain in self.ontology.keys():
                    for slot in self.ontology[domain]["slots"].keys():
                        domain_pos, slot_pos = -1, -1
                        domain_pos = 1
                        prefix_ids = [self.cls_token_id] + [self.domain_token_id] + self.domain_ids_lookup[domain]
                        slot_pos = len(prefix_ids)
                        prefix_ids += [self.slot_token_id] + self.slot_ids_lookup[slot]
                        if self.ontology[domain]['slots'][slot]['is_categorical']:
                            value_ids = self.value_ids_lookup[domain + "-" + slot]
                            prefix_ids += value_ids
                        ...
                        attn_mask = [1] * length
                        value_mask = [1 if x == self.value_token_id else 0 for x in input_ids]
                        ...
                        no_gpu_inst_slot.append({
                            "part_state_label": part_state_labels[name],
                            "state_label": state_labels["labels"][name],
                            "fixed_state_label": state_labels["fixed_labels"][name]
                        })

                        offset.append((did, uid, slot_id))
                        slot_id += 1

                seq_inst_dial.append(seq_inst_slot)
                int_inst_dial.append(int_inst_slot)
                no_gpu_inst_dial.append(no_gpu_inst_slot)
                uid += 1
        ...
        dont_num = 0
        update_num = 0
        for did, uid, sid in offset:
            if int_instances[did][uid][sid]["state_update"] == 0:
                none_num += 1
            elif int_instances[did][uid][sid]["state_update"] == 1:
                dont_num += 1
            elif int_instances[did][uid][sid]["state_update"] == 2:
                clear_num += 1
            else:
                update_num += 1

        print('update count:', none_num, clear_num, dont_num, update_num)

        print("utt num: ", self.utt_num)
        print("no span utt: ", self.no_span_utt)
        with open("test.json", "w") as f:
            json.dump(self.no_span_L, f, indent=4)
    ...
    def __getitem__(self, item):
        if self.evaluate:
            ...
            did, uid, sid = self.offset[item]
            return self.seq_instances[did][uid][sid], self.int_instances[did][uid][sid], self.no_gpu_instance[did][uid][sid]
    ...
    def collate(self, examples):
        ...
        return ids, inputs, domain_pos, slot_pos, value_mask, span_mask, labels
