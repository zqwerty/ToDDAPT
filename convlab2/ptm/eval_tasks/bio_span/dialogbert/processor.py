...
from convlab2.ptm.model.transformers import DialogBertTokenizer
from typing import List


class DialogBertProcessor:
    def __init__(self, pretrained_weights, labels_map):
        self.tokenizer = DialogBertTokenizer.from_pretrained(pretrained_weights)
        self.labels_map = labels_map
        self.tag_O_id = self.labels_map['label_to_id']['O']

    def batch_fn(self, batch_data):
        '''
        now the processor automatically decide which model to use
        :param batch_data: list of ('user'/'sys', tokens, tags, intents, dialog_act, context)
        :return: word_seq, word_mask, position_id, turn_id, role_id, tag_seq, tag_mask, intent
        '''
        ...
        position_id = torch.zeros((batch_size, max_seq_len), dtype=torch.long)
        turn_id = torch.zeros((batch_size, max_seq_len), dtype=torch.long)
        role_id = torch.zeros((batch_size, max_seq_len), dtype=torch.long)
        dial_ids = []
        turn_ids = []
        golden_update = []
        all_cate_labels = []
        ...
        for i in range(batch_size):
            ...
            position_id[i, :sen_len] = torch.LongTensor(batch_data[i]['position_ids'])
            turn_id[i, :sen_len] = torch.LongTensor(batch_data[i]['turn_ids'])
            role_id[i, :sen_len] = torch.LongTensor(batch_data[i]['role_ids'])
        ...
        return [{'word_seq_tensor': word_seq_tensor, 'word_mask_tensor': word_mask_tensor,
                'position_id_tensor': position_id, 'turn_id_tensor': turn_id, 'role_id_tensor': role_id,
            ...
            if tag.startswith('B'):
                ...
                while j < len(tags):
                    if tags[j].startswith('I') and tags[j][2:] == tag[2:]:
                        ...
                        # else:
                        #     # print('found multiple tag prediction for domain {} slot {}, value {}'.format(_domain, _slot, [
                        #     #     pred_state[_domain][_slot], decoded_value]))
                        #     continue
                        break
            i += 1
