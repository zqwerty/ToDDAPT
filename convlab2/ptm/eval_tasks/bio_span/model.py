...
class BERTNLU(torch.nn.Module):
    def __init__(self, mode='all', config_file='multiwoz_all_context.json',
        ...
        if not os.path.exists(os.path.join(data_dir, 'labels_map.json')):
            raise ValueError('data files not found, try preprocess')

        labels_map = os.path.join(data_dir, 'labels_map.json')
