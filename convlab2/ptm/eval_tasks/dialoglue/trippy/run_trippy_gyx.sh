...
model_name[0]="keep_60k-12.1-wm20k-lr1e-4-m0.99-bs32-t0.07-mlm_init-linear-ckpt4000"
pretrained_weights[0]="/home/guyuxian/data/dial-bert/moco/keep_60k-12.1-wm20k-lr1e-4-m0.99-bs32-t0.07-mlm_init-linear/checkpoint-4000/"

for ((i=0;i<${#pretrained_weights[@]};i++));
do
    DATA_DIR="../data_utils/dialoglue/multiwoz/MULTIWOZ2.1_fewshot"
    for fewshot_seed in 1 2 3 4 5;
    do
        echo "run on devices ${devices}"
        echo "${pretrained_weights[i]}"
        OUT_DIR="outputs/epoch50_fewshot_seed${fewshot_seed}/${model_name[i]}"
        echo ${OUT_DIR}
        mkdir -p ${OUT_DIR}
