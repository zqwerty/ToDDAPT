...
class DialogBertForNLU(nn.Module):
    def __init__(self, model_config, device, slot_dim, intent_dim, intent_weight):
        ...
        self.bert = DialogBertModel.from_pretrained(model_config.pretrained_weights)
    ...
    def forward(self, word_seq_tensor, word_mask_tensor, position_id_tensor=None, turn_id_tensor=None, role_id_tensor=None,
                tag_seq_tensor=None, tag_mask_tensor=None, intent_tensor=None):
        if self.model_config.probing:
            self.bert.eval()
            with torch.no_grad():
                outputs = self.bert(
                    input_ids=word_seq_tensor,
