...
def calculateAcc(predict_golden):
    intent_acc, slot_acc, overall_acc = [], [], []
    for item in predict_golden:
        intents = item['predict_intents']
        gold_intents = item['gold_intents']
        tags = item['predict_tags']
        gold_tags = item['gold_tags']
        intent_acc.append(sorted(intents)==sorted(gold_intents))
        slot_acc.append(tags==gold_tags)
        overall_acc.append(intent_acc[-1] and slot_acc[-1])
    return sum(intent_acc)/len(intent_acc), sum(slot_acc)/len(slot_acc), sum(overall_acc)/len(overall_acc)
