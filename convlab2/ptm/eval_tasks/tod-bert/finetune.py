...
# models
from models import *
from convlab2.ptm.pretraining.model import DialogBertModel, DialogBertTokenizer, DialogBertConfig
...
MODELS = {"bert": (BertModel, BertTokenizer, BertConfig),
          ...
          "dialogbert": (DialogBertModel, DialogBertTokenizer, DialogBertConfig),
...
if args["do_train"]:
    ...
    for run in range(args["nb_runs"]):
        ...
        if "dialog" in args['model_type']:
            if model.get_turn_embeddings_num() < args['max_turn_id']:
                logging.info("Resized max turn id to {}".format(args['max_turn_id']))
                model.resize_turn_embeddings(args['max_turn_id'])
        # model = DialogBertForDST(args)
        model = model.to(device)

        ## Create Dataloader
        trn_loader, trn_set = get_loader(args, "train", tokenizer, datasets, unified_meta)
        dev_loader, dev_set = get_loader(args, "dev", tokenizer, datasets, unified_meta,
                                         shuffle=args["task_name"] == "rs")
        tst_loader, tst_set = get_loader(args, "test", tokenizer, datasets, unified_meta,
                                         shuffle=args["task_name"] == "rs")
    ...
    # if MY_MODEL:
    dev_loader, _ = get_loader(args, "dev", tokenizer, datasets, unified_meta)
    tst_loader, _ = get_loader(args, "test", tokenizer, datasets, unified_meta, shuffle=args["task_name"] == "rs")
    model.eval()
