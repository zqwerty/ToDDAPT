...
class LastTurnMLMProcessor(TaskProcessor):
    def __init__(self, datasets, tokenizer:DialogBertTokenizer, mlm_probability=0.3, mlm_ignore_idx=-100, wwm=True):
        '''
        ...
        '''
        self.task_name = 'last_turn_mlm'
        self.data_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../prepare_data/processed_data')
    ...
    def load_data(self, train_batch_size, dev_batch_size, max_length=256, num_workers=0, use_multiwoz=False):
        ...
        for dataset in self.datasets:
            ...
            dev_sampler = DialogBertSampler(dev_dataset.data, dev_dataset.data_bucket_ids, dev_batch_size, drop_last=False, replacement=False)
            ...
            if dataset == 'multiwoz25' and not use_multiwoz:
                continue
            all_train_data += train_data

        train_dataset = MLMDataset(all_train_data, self.tokenizer, max_length)
    ...
    def collate_fn(self, batch_data):
        ...
        for i in range(batch_size):
            ...
            masked_input, label = self._last_turn_wwm_tokens(batch_data[i]['input_ids'], batch_data[i]['turn_ids'])
    ...
    def _last_turn_wwm_tokens(self, input_ids, turn_ids):
        masked_input = list(input_ids)
        not_mask_tokens = self.tokenizer.unique_added_tokens_encoder
        cand_indexes = []
        # tokens = []
        for i, (input_id, turn_id) in enumerate(zip(input_ids, turn_ids)):
            if turn_id < 1:
                continue
            elif turn_id > 1:
                break
            ...
            if token in not_mask_tokens:
                continue
            if self.wwm and len(cand_indexes) >= 1 and token.startswith("##"):
                cand_indexes[-1].append(i)
            else:
                cand_indexes.append([i])
        # print(cand_indexes)
        # print(tokens)
        # print([[tokens[x] for x in y] for y in cand_indexes ])
...
if __name__ == '__main__':
    ...
    processor = LastTurnMLMProcessor(['camrest'], tokenizer, wwm=True, mlm_probability=0.3)
