...
class RSDataset(Dataset):
    def __init__(self, data, tokenizer, max_length):
        ...
        for d in data:
            if len(d["dialogue"]) > 2:
                context, response = d["dialogue"][:-2], d["dialogue"][-2:-1]

                encoded_context = tokenizer.prepare_input_seq(context, last_role='user', max_length=max_length, return_lengths=True)
                encoded_response = tokenizer.prepare_input_seq(response, last_role='system', max_length=max_length, return_lengths=True)
                self.data_bucket_ids.append(encoded_context["length"] + encoded_response["length"])
                self.data.append((encoded_context, encoded_response))
...
class RSProcessor(TaskProcessor):
    ...
    def load_data(self, train_batch_size, dev_batch_size, max_length=256, num_workers=0, use_multiwoz=False):
        ...
        for dataset in self.datasets:
            ...
            dev_dataset = RSDataset(dev_data, self.tokenizer, max_length)
            ...
            if dataset != 'multiwoz25' or use_multiwoz:
                ...
                train_dataset = RSDataset(train_data, self.tokenizer, max_length)
    ...
    def collate_fn(self, batch_data):
        ...
        """
        # pprint(batch_data)
        ctxt_batch = [d[0] for d in batch_data]
        resp_batch = [d[1] for d in batch_data]
        batch_data = ctxt_batch + resp_batch
        batch_size = len(batch_data) # 2 times larger than the batch set outside
        ...
        resp_labels = torch.tensor(np.arange(batch_size // 2), dtype=torch.long) # // 2 for batch size set outside
        ...
        for i in range(batch_size):
            ...
            if i < batch_size // 2:
                ...
                label = [self.tokenizer.unk_token_id for _ in range(len(masked_input))]
        ...
        return {"attention_mask": attention_mask,
                ...
                "masked_lm_labels": masked_lm_labels, "resp_labels": resp_labels}
...
if __name__ == '__main__':
    ...
    processor = RSProcessor(['camrest'], tokenizer, mlm_probability=0.15)
