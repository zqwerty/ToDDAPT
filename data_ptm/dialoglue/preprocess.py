    ...
    if not os.path.exists(os.path.join(self_dir, 'data.zip')) or not os.path.exists(
        ...
        for dataset in ['clinc']:
            for split in ['train', 'val', 'test']:
                data = []
                with open(os.path.join(self_dir, 'dialoglue', dataset, '{}.csv'.format(split)), encoding='utf-8') as f:
                    for row in f:
                        data.append(row.split('","')[0])
                data = data[1:]
                data = [d.replace('"', '') for d in data]
                # print(dataset, data[:10])
                ptm_data.extend([convert_format('{}_{}'.format(dataset, idx), utt, dataset, split) for idx, utt in enumerate(data)])

        for dataset in ['banking', 'hwu']:
            for split in ['train', 'val', 'test']:
                data = []
                with open(os.path.join(self_dir, 'dialoglue', dataset, '{}.csv'.format(split)), encoding='utf-8') as f:
                    for row in f:
        ...
        for dataset in ['restaurant8k']:
            for split in ['train', 'val', 'test']:
                with open(os.path.join(self_dir, 'dialoglue', dataset, '{}.json'.format(split))) as f:
                    data = json.load(f)
        ...
        for dataset in ['top']:
            for split in ['train', 'eval', 'test']:
                sentences = []
                with open(os.path.join(self_dir, 'dialoglue', dataset, '{}.txt'.format(split))) as f:
                    for line in f:
                        sent = line.split('<=>')[0]
